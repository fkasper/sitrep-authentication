...
// PrintConfigCommand represents the command executed by "bio config".
...
func (cmd *PrintConfigCommand) Run(args ...string) error {
	...
	if err := config.Validate(); err != nil {
		return fmt.Errorf("%s. To generate a valid configuration file run `bio config > bio.conf`", err)
	}

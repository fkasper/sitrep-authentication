...
import (
	"flag"
	"fmt"
	"github.com/fkasper/sitrep-authentication/cmd/authentication/help"
	"github.com/fkasper/sitrep-authentication/cmd/authentication/run"
	"io"
	"log"
	"math/rand"
	"os"
...
func (m *Main) Run(args ...string) error {
	...
	default:
		return fmt.Errorf(`unknown command "%s"`+"\n"+`Run 'authenticationd help' for usage`+"\n\n", name)
	}
...
// VersionCommand represents the command executed by "authenticationd version".
...
func (cmd *VersionCommand) Run(args ...string) error {
	...
	// Print version info.
	fmt.Fprintf(cmd.Stdout, "authentication v%s (git: %s %s, built %s)\n", version, branch, commit, buildTime)
...
var versionUsage = `
usage: version
	version displays the authentication version, build branch and git commit hash
`
